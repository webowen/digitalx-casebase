...
function schemaConstrainedPrompt(input: ProviderInput) {
  return `${sourcePrompt(input.sourceText, input.sourceUrl, Boolean(input.file), input.researchMode)}
...
async function callDeepSeek(input: ProviderInput): Promise<ProviderResult> {
  ...
  if (!apiKey) {
    throw new AiProviderError("deepseek", "DeepSeek 服务尚未配置，请设置 DEEPSEEK_API_KEY。", 503, "deepseek_missing_key");
  }
  ...
  const model = process.env.DEEPSEEK_CASE_PARSER_MODEL?.trim() || DEEPSEEK_CASE_PARSER_MODEL;
  let response: Response;
  try {
    response = await fetch("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        model,
        messages: [{ role: "user", content: schemaConstrainedPrompt({ ...input, researchMode: false }) }],
    ...
    throw new AiProviderError("deepseek", "暂时无法连接DeepSeek服务，请稍后重试。", 502, "deepseek_unreachable");
  }
  ...
  try {
    ...
    throw new AiProviderError("deepseek", "DeepSeek返回了无法识别的响应。", 502, "deepseek_invalid_response");
  }
  if (!response.ok) throw classifyProviderError("deepseek", response.status, payload);
  ...
  return {
    ...
    fallbackUsed: false,
    fallbackReason: "",
    result: applyLocationFallback(parseJsonResult("deepseek", extractChatCompletionText(payload))),
  };
}
...
export async function parseCaseWithProvider(input: ProviderInput) {
  if (input.file && !input.sourceText.trim()) {
    const result = await callGemini({ ...input, researchMode: false });
    return input.researchMode
      ? {
          ...result,
          fallbackUsed: true,
          fallbackReason: "扫描PDF已使用Gemini完成基础识别；为控制费用，本次没有继续执行联网长文研究。",
        }
      : result;
  }
