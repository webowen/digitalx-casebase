...
DigitalX 是一套面向智慧城市从业者的案例采集、整理、检索、阅读与研判工具。当前仓库以 **V1.4 地图案例工作台正式版**为稳定界面，并进入 V1.5 内容底座建设。V1.5.0-alpha.5.1 保留“目录定位地图、点击地图 POI 打开文档”的两级交互：当前选中案例使用独立地图 Marker 与 InfoWindow，不再被点位聚合器吞并，项目信息卡会绑定真实坐标并随地图移动；广州 CIM、北京一网统管和杭州城市大脑交通治理进入六项人工终审；其余存量案例完成原生七章协议迁移并明确保持待扩写、待补证据状态。独立阅读器继续作为“沉浸阅读”入口，正式案例数据库和账号权限仍未接入。
