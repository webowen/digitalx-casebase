...
DigitalX 是一套面向智慧城市从业者的“真实案例资产库 + 地图 + AI 研究助手”。`knowledge-base/cases/` 是真实案例母库，系统在构建时读取每个 `case.md` 的 Frontmatter；只有 `content_status: published` 的案例进入公开目录、地图 POI 和详情阅读器。`lib/mock-cases.ts` 仅作为 legacy/dev 兼容数据保留。
...
- V1.5 Alpha.3存量案例迁移说明：[docs/13-V1.5.0-alpha.3存量案例迁移说明.md](docs/13-V1.5.0-alpha.3存量案例迁移说明.md)
- V1.5 Alpha.4标杆终审与媒体证据：[docs/14-V1.5.0-alpha.4标杆终审与媒体证据.md](docs/14-V1.5.0-alpha.4标杆终审与媒体证据.md)
- V1.5 Alpha.5批量迁移说明：[docs/15-V1.5.0-alpha.5批量迁移说明.md](docs/15-V1.5.0-alpha.5批量迁移说明.md)
...
| `/assets` | 极简案例资产工作台 |
| `/lab/ai-case-studio` | 封存的旧 AI 案例生产实验室 |
| `/admin` | 旧地址兼容入口，不再出现在主导航 |
...
当前公开案例优先来自 `knowledge-base/cases/` 的 published 资产；浏览器本地导入案例继续作为兼容能力保留。`lib/mock-cases.ts` 仅保留退役演示数据作为测试兼容资料，不新增模拟案例。
