...
DigitalX 是一套面向智慧城市从业者的案例采集、整理、检索、阅读与研判工具。当前仓库在 **V1.0 高保真可交互原型** 基础上增加了真实 AI 案例解析 Alpha：公开端、地图检索、案例阅读器、研读模式和管理工作台均可运行，管理端可将粘贴正文或 PDF 交给配置的 AI 服务生成结构化草稿。正式数据库、账号权限和云端文件存储尚未接入。
...
因此，换浏览器、换电脑或清理浏览器数据后，本地新增内容不会自动同步。管理端已真实接入 Gemini 与 OpenAI 双模型路由，默认使用 Gemini，支持粘贴正文和最大 8MB 的 PDF；AI 只生成草稿，字段证据、置信度和待核验项需要人工复核。网页自动抓取、独立 OCR 任务、原文件云端保存和跨设备同步尚未实现。
