...
DigitalX 是一套面向智慧城市从业者的案例采集、整理、检索、阅读与研判工具。当前仓库为 **V1.4 地图案例工作台正式版**：以全屏地图为主要画布，通过左侧多层案例目录、中央案例预览和右侧筛选分析联动公开案例；文本型 PDF 先在浏览器本地提取，基础结构化优先调用 DeepSeek，显式开启联网研究时由 Tavily Basic Search 搜索公开网页，再由 DeepSeek 生成可复核的结构化案例文章。案例图片证据可保存至 Sites 对象存储并进入人工复核，阅读器支持桌面双页、移动单页和 3D 翻书。正式案例数据库和账号权限仍未接入。
