...
test("real indexed cases use stable unique case_id identities", () => {
  assert.equal(indexedCaseStats.total, 1181);
  assert.equal(new Set(indexedCases.map((item) => item.id)).size, 1181);
...
test("indexed case POI statistics stay explicit", () => {
  assert.equal(indexedCaseStats.mappable, 1144);
  assert.equal(indexedCaseStats.withoutPoi, 37);
});
